#ifndef TESTSORTS_TEST_FUNCTIONS_H
#define TESTSORTS_TEST_FUNCTIONS_H
...
namespace ostreamVersion {
    void uniform_0to2_in_pow_31_std_sort(const std::uint32_t N, const short SORT_TYPE) {
        ...
        std::cout << "vec[i] = array[i] ~ U(0, 2^31)" << std::endl;
        for (auto i = 0u; i < 5; ++i) {
            for (auto j = 0u; j<N; ++j) {
            ...
            std::cout << "sorted in " << testDataArray[i] << "ms" << std::endl;
            ...
            std::cout << "sorted in " << testDataVector[i] << "ms" << std::endl;
        }
        std::cout << "Average (array): " << std::accumulate(testDataArray, testDataArray + 5, 0l) / 5. << "ms"  << std::endl;
        std::cout << "Average (vector): " << std::accumulate(testDataVector, testDataVector + 5, 0l) / 5. << "ms" << std::endl << std::endl;
    }

    void uniform_0to2_in_pow_15_std_sort(const std::uint32_t N, const short SORT_TYPE) {
        std::uint32_t array[N];
        std::vector<std::uint32_t> vec(N);
        std::random_device rd;
        std::mt19937 gen(rd());
    ...
    void uniform_0to2_N_1_std_sort(const std::uint32_t N, const short SORT_TYPE) {
        std::uint32_t array[N];
        std::vector<std::uint32_t> vec(N);
        std::random_device rd;
        std::mt19937 gen(rd());
        ...
        long testDataVector[5];
        auto now = std::chrono::high_resolution_clock::now;
        std::cout << "vec[i] = array[i] ~ U(0, " << N - 1 << ")" << std::endl;
        for (auto i = 0u; i < 5; ++i) {
            for (auto j = 0u; j<N; ++j) {
                vec[j] = array[j] = dis(gen);
            }
    ...
    void normal_0to2_in_pow_31_std_sort(const std::uint32_t N, const short SORT_TYPE) {
        std::uint32_t array[N];
        std::vector<std::uint32_t> vec(N);
        std::random_device rd;
        std::mt19937 gen(rd());
        ...
        long testDataVector[5];
        auto now = std::chrono::high_resolution_clock::now;
        std::cout << "vec[i] = array[i] ~ N(2^31/2, 2^31)" << std::endl;
        for (auto i = 0u; i < 5; ++i) {
            for (auto j = 0u; j<N; ++j) {
                vec[j] = array[j] = dis(gen);
            }
...
#endif //TESTSORTS_TEST_FUNCTIONS_H
